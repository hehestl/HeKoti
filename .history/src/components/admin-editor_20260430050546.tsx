...
export function AdminEditor({
  ...
  const cloneToLanguage = async (targetLang: string) => {
    ...
    if (res.status === 409 && body.code === "exists" && body.existingPath) {
      window.location.href = `/${targetLang}/admin?tab=posts&activePath=${encodeURIComponent(body.existingPath)}`;
      return;
    }
    if (!res.ok || !body.path) {
    ...
    window.location.href = `/${targetLang}/admin?tab=posts&activePath=${encodeURIComponent(body.path)}`;
  };
