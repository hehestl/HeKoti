    ...
    <form
      ...
      onSubmit={async (event) => {
        ...
        if (step === "password") {
          ...
          if (!res.ok) {
            setError(body.message ?? dict.admin.auth.failed);
            return;
          }
        ...
        if (!res.ok) {
          setError(body.message ?? dict.admin.auth.invalidCode);
          return;
        }
        window.location.href = `/${lang}/admin`;
      }}
    >
      {step === "password" ? (
        <>
          <input
            style={inputStyle}
            placeholder={dict.admin.auth.login}
            value={email}
          ...
          <input
            style={inputStyle}
            placeholder={dict.admin.auth.password}
            type="password"
            value={password}
        ...
        <>
          <p style={{ margin: 0, fontSize: 13, color: "var(--muted)", lineHeight: 1.45 }}>
            {dict.admin.auth.totpDesc}
          </p>
          <input
            style={inputStyle}
            placeholder={dict.admin.auth.totpCode}
            inputMode="numeric"
          ...
          >
            {dict.admin.auth.back}
          </button>
        </>
      )}
      <button style={buttonStyle} disabled={step === "password" ? !email || !password : !totpCode}>
        {dict.admin.auth.signIn}
      </button>
