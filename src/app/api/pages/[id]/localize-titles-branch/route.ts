...
import { localizeTitlesBranch } from "@/lib/page-localize";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { pageCount, results } = await localizeTitlesBranch({
      rootPageId: id,
      editorId: user.id,
    ...
    return NextResponse.json({
      ok: true,
      pageCount,
      summary: { updated, created, failed, total: results.length },
      results,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Title localization failed";
