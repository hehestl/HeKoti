...
import { FileTask, InsertablePage } from '@hehestl/db/types/entity.types';
import { markdownToHtml } from '@hehestl/editor-ext';
...
export class FileImportTaskService {
  ...
  async processGenericImport(opts: {
    ...
    const hehestlMetadata = await readhehestlMetadata(extractDir);
    ...
    for (const absPath of allFiles) {
      ...
      const pageMetadata = hehestlMetadata?.pages[encodedPath];
    ...
    sortedFolders.forEach((folderPath) => {
      ...
      if (!pagesMap.has(mdPath) && !pagesMap.has(htmlPath)) {
        ...
        if (!matched) {
          ...
          const placeholderMetadata = hehestlMetadata?.pages[encodedMdPath];
          pagesMap.set(mdPath, {
            id: v7(),
            slugId: generateSlugId(),
    ...
    const sortSiblings = (siblings: ImportPageNode[]) => {
      if (hehestlMetadata) {
        siblings.sort((a, b) => {
          const posA =
            hehestlMetadata.pages[encodedPathsMap.get(a.filePath)]?.position;
          const posB =
            hehestlMetadata.pages[encodedPathsMap.get(b.filePath)]?.position;
          if (posA && posB) {
