...
import { sanitizeUrl } from "@hehestl/editor-ext";
import { parseHeaderNavMenusFromJson } from "@/lib/header-nav-config.ts";

export type { HeaderNavMenuGroup } from "@/lib/header-nav-config.ts";
...
/** JSON array of `{ title, items: [{ label, url }] }` from env `HEADER_NAV_MENUS` / window.CONFIG */
export function getHeaderNavMenuGroups() {
  return parseHeaderNavMenusFromJson(getConfigValue("HEADER_NAV_MENUS"));
}
